...
var dActual = "10.6.40.160" //ip nodo actual
...
var contadorMensajesC int  //mensajes de datanode A
var mutexCC = &sync.Mutex{}
...
func (s *server) RequestRA(ctx context.Context, p *book.ExMutua) (*book.ACK, error){
	...
	mutexCC.Lock()
	contadorMensajesC +=1 // SUMA UN MENSAJE PARA METRICAS DEL INFORME
	mutexCC.Unlock()
...
func RicAwla(prop *book.PropuestaLibro){
	...
	if prop.DatanodeA{
		...
		connDNA, err := grpc.Dial(dA + ":50503", grpc.WithInsecure())
		if err != nil{
		...
		if ACK.Ok == "ok"{
			log.Println("Respuesta recibida de Nodo A")
			resp1 = true
		}
	...
	if prop.DatanodeB{
		...
		connDNB, err := grpc.Dial(dB + ":50505", grpc.WithInsecure())
		if err != nil{
			log.Printf("No se pudo conectar a DataNode: %v", err)
		}
		defer connDNB.Close()
		...
		if ACK.Ok == "ok"{
			log.Println("Respuesta recibida de Nodo B")
			resp2 = true
		}
	...
	if (prop.DatanodeA && resp1) || (prop.DatanodeB && resp2){
...
func (s *server) EnviarChunkDN(ctx context.Context, chunk *book.Chunk) (*book.Chunk, error){
	...
	file, err := os.Open("./DNC/Chunks/" + chunk.NombreArchivo)
	defer file.Close()
	if (err != nil){
...
func (s *server) RecibirChunksDN(ctx context.Context, c *book.Chunk) (*book.ACK, error){
	...
	f, err := os.OpenFile("./DNC/Chunks/" + c.NombreArchivo, os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0644)
...
func enviarChunk(archivoChunk string, ip string){
	...
	file, err := os.Open("./DNC/Chunks/" + archivoChunk)
	...
	//eliminar archivo
	file.Close()
	err = os.Remove("./DNC/Chunks/" + archivoChunk)
...
func analizarPropuesta(prop *book.PropuestaLibro) (bool, string){
	...
	if (prop.DatanodeA && !checkDatanode(dA, "50503", "datanode A")){
		fmt.Println("Se rechaza la propuesta\n")
		return false, dA
	}
	if (prop.DatanodeB && !checkDatanode(dB, ":50505", "datanode B")){
		fmt.Println("Se rechaza la propuesta\n")
		return false, dB
	} 
...
func enviarPropNNDes(prop *book.PropuestaLibro) {
	...
	//conexión
	connNN, err := grpc.Dial(dNN + ":50508", grpc.WithInsecure(), grpc.WithBlock())
	...
	log.Println("Cantidad de mensajes:", contadorMensajesC)
}
...
func (s *server) RecibirChunks(stream book.BookService_RecibirChunksServer) error {
	...
	for {
		...
		neoArchLibro := "./DNC/Chunks/" + chunk.NombreLibro + "_" + strNumChunk
...
//DESCENTRALIZADO
		...
		respA, DNcaidoA := enviarPropuestaDN(dA, ":50503", &book.PropuestaLibro{Propuesta: prop, DatanodeA: a, DatanodeB: b, DatanodeC: c})
		respB, DNcaidoB := enviarPropuestaDN(dB, ":50505", &book.PropuestaLibro{Propuesta: prop, DatanodeA: a, DatanodeB: b, DatanodeC: c})
...
func serveCD(){
	...
	listenCD, err := net.Listen("tcp", dActual + ":50515")
...
func serveCU(){
	...
	listenCU, err := net.Listen("tcp", dActual + ":50519")
...
func serveNN(){
	...
	listenNN, err := net.Listen("tcp", dActual + ":50511")
	failOnError(err, "Error de conexión con Cliente Downloader")
...
func serveDNA(){
	...
	listenDNA, err := net.Listen("tcp", dActual + ":50502")
...
func serveDNB(){
	...
	listenDNB, err := net.Listen("tcp", dActual + ":50504")
